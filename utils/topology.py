...
class TopologyProcessor:
    ...
    def compute_persistence_diagram_graph(self, data, return_lifetimes=False):
        ...
        try:
            ...
            if np.isinf(dist_matrix).any():
                ...
                return self._zero_vector(return_lifetimes)
    ...
    def _zero_vector(self, return_lifetimes):
        zero = torch.zeros(self.topo_dim)
        return (zero, []) if return_lifetimes else zero
    ...
    def _get_largest_connected_subgraph(self, G):
        ...
        return nx.relabel_nodes(subgraph, node_mapping)
    ...
    def _extract_topological_features(self, dist_matrix, return_lifetimes=False):
        ...
        tensor_feats = torch.tensor(feats, dtype=torch.float)
        return (tensor_feats, lifetimes) if return_lifetimes else tensor_feats
    ...
    def persistence_to_vector(self, dgm, return_lifetimes=False):
        ...
        h_counts = Counter(dim for dim, (birth, death) in dgm
                           if dim in [0, 1] and not np.isinf(death))
        ...
        return (padded, features, h_counts) if return_lifetimes else padded
    ...
    def summarize_statistics(self, output_path=None):
        ...
        summary = {f"H{dim}_avg_count": self.homology_counts[dim] / self.total_graphs
                   for dim in self.homology_counts}
